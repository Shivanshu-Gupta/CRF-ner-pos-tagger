...
from param import datasets as all_datasets, model_types, ray_dir, TaggingParams
...
def df_search_results(datasets=all_datasets, models=model_types, encs=[0, 1, 2],
                      metric='val_accuracy', add_param_cols=True, hyperopt=False):
...
def get_best_configs(datasets=all_datasets, models=model_types, encs=[0, 1, 2],
                     metric='val_accuracy', hyperopt=False):
    best_configs = {}
    for dataset in datasets:
        for model in models:
            for emb in [True, False]:
                for enc in encs:
                    best_config = get_best_config(dataset, model, emb, enc, metric,
                                                  structure=True, hyperopt=hyperopt)
                    if best_config is None:
                        continue
